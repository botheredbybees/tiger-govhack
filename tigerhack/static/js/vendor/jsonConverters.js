...
(function() {
    ...
    // some new stuff for converting from meters to lat long using the Proj4js library
    
			var source = new Proj4js.Proj('EPSG:3857');    // for handling the 'in meters' stuff from theLIST
			var dest = new Proj4js.Proj('EPSG:4326');			 // for producing nice degrees and minutes
                ...
                if (esriGeom.x) {
                    ...
                    for (i = 0; i < geomParts.length; i++) {
                        ...
                        if (ringIsClockwise(ring)) {
                        		// convert to human coordinates - Peter
				                    rLength = ring.length;
				                    for (j=0; j < rLength; j++) {
				                    	var p = new Proj4js.Point(ring[j][0],ring[j][1]);
